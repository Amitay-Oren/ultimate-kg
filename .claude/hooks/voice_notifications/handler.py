...
def play_voice_sound(voice: str = "ding", sound_name: str = "task_complete") -> None:
    ...
    try:
        ...
        # Configure SDL audio driver with fallbacks for different environments
        audio_drivers = ['pulse', 'alsa', 'dsp', 'oss']
        audio_driver_used = None
        ...
        for driver in audio_drivers:
            try:
                ...
                # Initialize pygame mixer with current driver
                pygame.mixer.pre_init(frequency=22050, size=-16, channels=2, buffer=1024)
                pygame.mixer.init()
                
                # Test if initialization was successful
                if pygame.mixer.get_init():
                    audio_driver_used = driver
                    logger.debug(f"Successfully initialized audio with driver: {driver}")
                    break
                else:
                    pygame.mixer.quit()
                    
            except Exception as e:
                logger.debug(f"Audio driver {driver} failed: {e}")
                try:
                    pygame.mixer.quit()
                except:
                    pass
        ...
        if not audio_driver_used:
            logger.warning("All pygame audio drivers failed, trying Windows audio fallback")
            # Try Windows PowerShell audio as last resort
            try:
                import subprocess
                import shutil
                
                if shutil.which('powershell.exe'):
                    logger.debug("Attempting Windows PowerShell audio playback")
                    # Convert WSL path to Windows path
                    windows_path = str(sound_path).replace('/mnt/c/', 'C:\\').replace('/', '\\')
                    logger.debug(f"Windows path: {windows_path}")
                    
                    # Use Windows PowerShell to play MP3 via Windows Media Player
                    ps_command = f'''
                    Add-Type -AssemblyName presentationCore
                    $mediaPlayer = New-Object System.Windows.Media.MediaPlayer
                    $mediaPlayer.Open([System.Uri]::new("{windows_path}"))
                    $mediaPlayer.Play()
                    ...
                    if result.returncode == 0:
                        logger.info(f"✅ Successfully played via Windows PowerShell: {voice}/{sound_name}")
                        return
                    else:
                        logger.debug(f"PowerShell audio failed: {result.stderr}")
            except Exception as e:
                logger.debug(f"Windows PowerShell fallback failed: {e}")
            
            logger.warning("All audio methods failed, falling back to terminal bell")
            raise Exception("Could not initialize any audio driver")
